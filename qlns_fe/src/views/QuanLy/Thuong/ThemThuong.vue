        ...
        <v-card-text>
            ...
            <v-form class="mt-3" ref="formValidate">
                ...
                <v-layout row wrap justify-space-around>
                    <v-flex md5 class="">
                        <v-text-field v-model="Thuong.LoaiThuong" label="Loại Thưởng" required :rules="EmptyRules">
                        </v-text-field>
                    </v-flex >
                    <v-flex md5>
                        <v-text-field v-model="Thuong.MucThuong" required :rules="NumberRules" label="Mức Thưởng"  >
                        </v-text-field>
                    </v-flex>
    ...
    methods:{
        ...
        async saveProduct(){
            if(this.$refs.formValidate.validate()){
                ...
                if(this.Thuong.MaThuong==undefined){
                    this.loading=true;
                    await axios.post("https://localhost:5001/api/Thuongs",this.Thuong);
                    console.log(this.Thuong)
                    this.Thuong = {};
                    this.$emit('closePopup',false);
                }
                    ...
                    this.loading=true;
                    console.log(this.Thuong)
                    await axios.put("https://localhost:5001/api/Thuongs",this.Thuong);
                    this.Thuong = {};
                    this.$emit('closePopup',false);
                }
                this.$emit("loadDL");
