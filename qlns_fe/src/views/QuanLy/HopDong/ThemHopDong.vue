        ...
        <v-card-text>
            ...
            <v-form class="mt-3" ref="formValidate">
                ...
                <v-layout row wrap justify-space-around>
                    <v-flex md5 class="">
                        <v-text-field v-model="HopDong.NoiDung" label="Nội Dung" required :rules="EmptyRules">
                        </v-text-field>
                    </v-flex >
                    <v-flex md5>
                        <v-text-field v-model="HopDong.GhiChu" required :rules="EmptyRules" label="Ghi Chú"  >
                        </v-text-field>
                    </v-flex>
    ...
    methods:{
        ...
        async saveProduct(){
            if(this.$refs.formValidate.validate()){
                ...
                if(this.HopDong.MaHopDong==undefined){
                    this.loading=true;
                    await axios.post("https://localhost:5001/api/HopDongs",this.HopDong);
                    console.log(this.HopDong)
                    this.HopDong = {};
                    this.$emit('closePopup',false);
                }
                    ...
                    this.loading=true;
                    console.log(this.HopDong)
                    await axios.put("https://localhost:5001/api/HopDongs",this.HopDong);
                    this.HopDong = {};
                    this.$emit('closePopup',false);
                }
                this.$emit("loadDL");
