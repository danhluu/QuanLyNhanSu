...
<template>
  <v-container>
    ...
    <ThemPhongBan :PhongBan="PhongBan" :dialogUpdate="dialog" @closePopup="closePopup" @loadDL="loadDL"/>
    ...
    >
      <template  v-slot:item="{ item }">
        ...
        <tr class="mb-3 body-1" style="">
          <td style="width:10%;">{{item.MaPhongBan}}</td>
          ...
          <td style="width:15%;">
            ...
            <v-btn @click="DeleteData(item.MaPhongBan)" class="primary dark white--text ml-2">Xoá</v-btn>
          </td>
        </tr>
      </template>
    </v-data-table>
    <XoaPhongBan :dialogDelete="dialogDelete" @closeDeletePopup="closeDeletePopup" :MaPhongBan="MaPhongBan" @loadDL="loadDL"/>
  <!-- <v-data-table
    :headers="headers"
    :items="nhanviens"
    :items-per-page="5"
...
import ThemPhongBan from "../../QuanLy/PhongBan/ThemPhongBan"
import XoaPhongBan from "../../QuanLy/PhongBan/XoaPhongBan"
  export default {
    components:{
      NavBarQL,
      ThemPhongBan ,
    ...
    data () {
      return {
        ...
        search: '',
        headers: [
          { text: 'Mã Phòng Ban', value: 'MaPhongBan', class:'text-lg-h6 font-weight-bold' },
          {
            text: 'Tên Phòng Ban',
            align: 'start',
            // sortable: false,
    ...
    async created(){
      const response = await axios.get("https://localhost:5001/api/PhongBans");
