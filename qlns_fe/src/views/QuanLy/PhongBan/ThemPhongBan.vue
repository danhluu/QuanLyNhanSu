        ...
        <v-card-text>
            ...
            <v-form class="mt-3" ref="formValidate">
                ...
                <v-layout row wrap justify-space-around>
                    <v-flex md5 class="">
                        <v-text-field v-model="PhongBan.TenPhongBan" label="Tên Phòng ban" required :rules="EmptyRules">
                        </v-text-field>
                    </v-flex >
                    <v-flex md5>
                        <v-text-field v-model="PhongBan.sdt" required :rules="EmptyRules" label="Số Điện Thoại"  >
                        </v-text-field>
                    </v-flex>
    ...
    methods:{
        ...
        async saveProduct(){
            if(this.$refs.formValidate.validate()){
                ...
                if(this.PhongBan.MaPhongBan==undefined){
                    this.loading=true;
                    await axios.post("https://localhost:5001/api/PhongBans",this.PhongBan);
                    console.log(this.PhongBan)
                    this.PhongBan = {};
                    this.$emit('closePopup',false);
                }
                    ...
                    this.loading=true;
                    console.log(this.PhongBan)
                    await axios.put("https://localhost:5001/api/PhongBans",this.PhongBan);
                    this.PhongBan = {};
                    this.$emit('closePopup',false);
                }
                this.$emit("loadDL");
